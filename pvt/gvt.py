...
class GroupAttention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        if pad_r > 0 or pad_b > 0:
            x = x.view(B, Hp, Wp, C)
            x = x[:, :H, :W, :].contiguous().reshape(B, N, C)
...
class PVT(Backbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], block_cls=Block, out_features=None):
        super().__init__()
...
class PCPVT(PVT):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
                 ...
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1], block_cls=SBlock, out_features=None):
        super(PCPVT, self).__init__(img_size, patch_size, in_chans, num_classes, embed_dims, num_heads,
                                    ...
                                    norm_layer, depths, sr_ratios, block_cls, out_features)
...
class ALTGVT(PCPVT):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 num_heads=[2, 4, 8, 16], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[2, 2, 10, 4], sr_ratios=[8, 4, 2, 1], block_cls=GroupBlock, wss=[7, 7, 7, 7], out_features=None):
...
def build_gvt_backbone(cfg, input_shape):
    """
    ...
    return ALTGVT(
        ...
        drop_rate=0.0,
        wss=cfg.MODEL.PVT.WSS,
        out_features=out_features
    )

@BACKBONE_REGISTRY.register()
def build_gvt_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
    ...
    """
    bottom_up = build_gvt_backbone(cfg, input_shape)
...
@BACKBONE_REGISTRY.register()
def build_retinanet_gvt_fpn_backbone(cfg, input_shape: ShapeSpec):
    """
    Args:
        cfg: a detectron2 CfgNode
    Returns:
